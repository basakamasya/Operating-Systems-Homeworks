...
public class Philosopher extends Thread {
	...
	public void take_forks(int i) {
		try {
			mutex.acquire(); //protect the critical region with mutex
			state[i] = HUNGRY; //philosopher becomes hungry
			table.Hungry_GUI(id);
			test(i); //testing if forks are available
			mutex.release();
			sem[i].acquire(); //can eat if test was successful
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void put_forks(int i) {
		try {
			int LEFT = (i + N - 1) % N; //computing the id's of left and right philosophers
			int RIGHT = (i + 1) % N;
			mutex.acquire(); //protect the critical region by mutex
			state[i] = THINKING;
			table.StopEating_GUI(id);
			table.ForkPut_GUI(id);
			test(LEFT); //testing if left philosopher is hungry, unblock him
			test(RIGHT); //testing if right philosopher is hungry, unblock him
			mutex.release();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void run() {
		...
		int walktime = new Random().nextInt(10) + 1; //random number between 1 and 10
		walktime = walktime * 1000; //convert to milliseconds
		try {
			sleep(walktime); //thread sleeps to represent walking time
			table.PutPlate_GUI(id);
			// Barrier
			for (int a = 0; a < 5; a++) {
				barriers[a].release(); //does up on every barrier
			}
			try {
				for (int b = 0; b < 5; b++) {
					barriers[id].acquire(); //does down on himself for all other philosophers who did up
				}
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			...
			state[id] = THINKING;
			while (true) {
				int thinktime = new Random().nextInt(11);
				thinktime = thinktime * 1000;
				sleep(thinktime); //sleeps for random time between 0 and 10 seconds, representing thinking time
				take_forks(id);
				table.ForkTake_GUI(id);
				table.Eating_GUI(id); //eating
				put_forks(id);
			}
		} catch (InterruptedException e) {
			e.printStackTrace();
